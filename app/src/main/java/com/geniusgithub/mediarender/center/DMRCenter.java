...
import com.geniusgithub.mediarender.util.MediaTypeJudgeUtil;
...
public class DMRCenter implements ActionReflectionListener, IDMRAction{
	...
	private MediaTypeJudgeUtil mediaTypeJudgeUtil;
	...
	public DMRCenter(Context context){
		mContext = context;
		mediaTypeJudgeUtil = MediaTypeJudgeUtil.getInstance();
	}
	...
	public void onRenderAvTransport(String value, String data) {
		...
		Log.e("onRenderAvTransport: ", value);

		// 播放类型
		try {
			mediaTypeJudgeUtil.getRequestHeader(value);
			switch (mediaTypeJudgeUtil.getMediaTyle()) {
				case CUR_MEDIA_TYPE_VIDEO:
					mediaInfo.setObjectClass(DlnaUtils.DLNA_OBJECTCLASS_VIDEOID);
					break;
				case CUR_MEDIA_TYPE_MUSCI:
					mediaInfo.setObjectClass(DlnaUtils.DLNA_OBJECTCLASS_MUSICID);
					break;
				case CUR_MEDIA_TYPE_PICTURE:
					mediaInfo.setObjectClass(DlnaUtils.DLNA_OBJECTCLASS_PHOTOID);
					break;
				default:
					break;
			}
